check_epochlen_is_60 <- function(agdb, algorithm) {
  if (attr(agdb, "epochlength") != 60)
    stop("Epochs should have length 60s to apply ", algorithm, ". Epochs can be aggregated with `collapse_epochs`.")
}
...
check_no_missing_timestamps <- function(agdb) {
  if (has_missing_epochs(agdb))
    stop("Missing timestamps. Epochs should be evenly spaced from first(timestamp) to last(timestamp).")
}
...
check_no_missing_state <- function(agdb) {
  if (!exists("sleep", agdb))
    stop("Missing asleep/awake (0/1) indicator column. These states can be inferred with `apply_sadeh` or `apply_cole_kripke.`")
  if (anyNA(agdb$sleep))
